...
class ModelExtensionPaymentCardGateSpraypay extends Model {
    ...
    public function getMethod( $address, $total ) {
        ...
        if (!checkPaymentCurrency($currency, 'cardgatespraypay')) {
            return [];
        }
        ...
        if ( $this->config->get('payment_cardgatespraypay_total') > 0 && $this->config->get('payment_cardgatespraypay_total') > $total) {
            $status = false;
