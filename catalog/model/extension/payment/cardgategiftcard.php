...
class ModelExtensionPaymentCardGateGiftcard extends Model {
    ...
    public function getMethod( $address, $total ) {
        ...
        if ($this->config->get('payment_cardgate_use_logo') == 1){
            $title = '<img style="height:100%; width:100%; max-height: 30px; max-width: 60px;" src="image/payment/cgp/giftcard.svg" /> ';
        }
        ...
        if ($this->config->get('payment_cardgategiftcard_custom_payment_method_text')){
            $title .= $this->config->get('payment_cardgategiftcard_custom_payment_method_text').' ';
        }
